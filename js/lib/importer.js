...
var ImporterModule  = {
	...
	// Mod: remove skip dialog and add own on_complete
	processFileList: function( files, options, oncomplete )
	{
		options = options || {};
		var that = this;
		...
		function inner_process( file, options ){
			...
			ImporterModule.processResource( filename, file, that.getImporterOptions( filename ), (filename, resource) => oncomplete(filename, resource));
		}

		//one single file shows the dialog
		var file = files[0];
		var callback = inner_process;
