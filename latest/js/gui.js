...
GUI.prototype.addNodeInspector = function(node, widgets, options)
{
	...
	*/
	...
	var tab = document.createElement('span');
	tab.className = "side-tab" + (node._lastTab || options.selectedComponent ? "" : " selected");
	tab.innerHTML = "<img src=" + icon + " style='width: 100%;'>"
	tab.addEventListener('click', function(){
		$(".side-tab").removeClass("selected")
		this.classList.add( "selected" );
	...
	if(node.components)
		for (let c in node.components)
		{
			...
			if( component.mark && component.create )
			{
				var icon = component.constructor.icon;
				var tab = document.createElement('span');
				tab.className = "side-tab" + ((node._lastTab && node._lastTab == c) || (c === options.selectedComponent) ? " selected" : "");
				tab.innerHTML = "<img src=" + icon + " style='width: 100%;'>";
				tab.addEventListener('click', function(){
					$(".side-tab").removeClass("selected")
					this.classList.add( "selected" );
	...
	node_widgets.on_refresh = (function(name)
	{
		...
		else
		{
			...
			if( component && component.mark && component.create )
				component.create( node_widgets );
		}
	...
	node_widgets.on_refresh( options.selectedComponent );
}
    ...
    widgets.on_refresh = function(){
		...
		widgets.addButton(null, "Add", {callback: function(){
			...
			if(selected) {
				...
				node._lastTab = null;
				that.updateSidePanel(null, node.name, { selectedComponent: selected.short_name });
				dialog.close();
			}
