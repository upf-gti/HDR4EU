// NOT UPDATED; DO NOT USE THIS VERSION

// init uniforms
// load HDRE and upload environment to GPU
// update scene nodes
...
var PBR = {
    ...
    getUniforms(r, m)
    {
        return {
            ...
            "u_roughness": r === undefined ? 1.0 : r,
            "u_metalness": m === undefined ? 1.0 : m,
            "u_alpha": 1.0,
    ...
    setHDRE( file, on_complete )
    {
        ...
        LiteGUI.request({dataType: "arraybuffer", url: file, success: function(result){
            ...
            var header = that.parseHDRE(result, filename);

            if(on_complete)
                on_complete(filename, header);
		}});
    },
